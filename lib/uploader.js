...
const debug = require('debug')('loader:directuploader');
...
function Uploader(req, res) {
    ...
    this.upload = (done) => {
        ...
        return steps.unzipUpload(passon)
            ...
            .catch(err => {
                debug('[%s] Rejection or unhandled failure during upload:\n\t%s\n\t%s',
                    passon.id, JSON.stringify(err), util.inspect(passon, { depth: 1, colors: true }));
                let status = 500;
                if (err.status) {
                    status = err.status;
                }
