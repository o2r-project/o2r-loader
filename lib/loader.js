...
function Loader(req, res) {
    ...
    this.loadOwncloud = (done) => {
        ...
        return steps.publicShareLoad(passon)
            ...
            .catch(err => {
                debug('Rejection or unhandled failure during load from public share: %s\n%s', JSON.stringify(err), err);
                let status = 500;
                if (err.status) {
                    status = err.status;
                }
    ...
    this.loadZenodo = (done) => {
        ...
        return steps.checkZenodoContents(passon)
            ...
            .catch(err => {
                debug('Rejection or unhandled failure during load from zenodo: %s\n%s', JSON.stringify(err), err);
                let status = 500;
                if (err.status) {
                    status = err.status;
                }
