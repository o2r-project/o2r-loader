...
function Loader(req, res) {
    ...
    this.loadOwncloud = (done) => {
        debug('Handling public share load from "%s" for user %s', this.req.body.share_url, this.req.user.orcid);
        ...
        return steps.publicShareLoad(passon)
            .then(steps.unzipCommandFromShare)
            .then(steps.unzip)
            ...
            .catch(err => {
                debug('Rejection or unhandled failure during load: \n\t%s',
                    JSON.stringify(err));
                let status = 500;
                if (err.status) {
                    status = err.status;
