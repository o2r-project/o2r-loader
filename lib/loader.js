...
function Loader(req, res) {
    ...
    this.load = (done) => {
        debug('Handling upload of %s for user %s', this.req.body.shareURL, this.req.user.orcid);

        let passon = {
            //TODO: generate(?) id, add shareURL/token/host
            id: randomstring.generate(c.id_length),
            shareURL: this.req.body.shareURL,
        ...
        return steps.publicShareLoad(passon)
            ...
            .catch(err => {
                debug('[%s] Rejection or unhandled failure during execute: \n\t%s',
                    this.passon.id, JSON.stringify(err));
                let status = 500;
                if (err.status) {
                    status = err.status;
                }
