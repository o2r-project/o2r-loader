...
function publicShareLoad(passon) {
    //return new Promise((fulfill, reject) => {
        ...
        let token = url.parse(passon.share_url).path.split('/')[3];
    ...
    return new Promise((fulfill, reject) => {
        ...
        for (i = 0; i < contents.length; i++) {
            ...
            if (contents[i].mime === 'application/zip') { //todo: mime does not always exist -> check if it does
                result.zipCount++;
                result.zipName = contents[i].filename;
            }
...
function checkAndCopyCompendium(passon) {
    return new Promise((fulfill, reject) => {
        if (passon.result.bagitCount === 0) {
            ...
            let download_url = passon.share_url + '/download?path=' + encodeURIComponent(passon.webdav_path);
            var zip_path = path.join(config.fs.incoming, passon.id);
            var cmd = 'wget -q -O ' + zip_path + ' ' + download_url;
...
function checkAndLoadZip(passon) {
    ...
    return new Promise((fulfill, reject) => {
        if (passon.result.zipCount === 1 && passon.bagitCount === 0) {
            return passon.client
                .getFileContents(passon.result.zipName)
                .then(function (zipData) {
                    //change to filename
                    passon.zip_path = path.join(config.fs.incoming, passon.id);
                    fs.writeFileSync(passon.zipPath, zipData);
                })
        } else {
            fulfill(passon);
        }
