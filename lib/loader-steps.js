    ...
    return new Promise((fulfill, reject) => {
        ...
        passon.client
            ...
            .then(function (contents) {
                // create direcotry
                fs.mkdir(path.join(localPath, remotePath, contents[i].basename), function (err) {
                    if (err) {
                        reject(err);
                    }
                    ...
                    for (i = 0; i < contents.length; i++) {
                        switch (contents[i].type) {
                            case 'file':
                                passon.client
                                    .getFileContents(contents[i].basename)
                                    .then(function (file) {
                                        fs.writeFile(path.join(localPath, remotePath, contents[i].basename), file);
                                    });
                                break;
                            case 'directory':
                                // start syncWebdav for subdirectory
                                syncWebdav(contents[i].filename, localPath, (err) => {
                                    if (err) {
                                        reject(err);
                                    }
                                });
...
function publicShareLoad(passon) {
    return new Promise((fulfill, reject) => {
        ...
        */
        ...
        // todo: add path parameter: .getDirectoryContents(path)
...
function copyPublicShare(passon) {
    return new Promise((fulfill, reject) => {
        ...
        syncWebdav('', path.join(config.fs.compendium, passon.id), (err) => {
            if (err) {
