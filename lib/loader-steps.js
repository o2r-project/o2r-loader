...
const debug = require('debug')('loaderr');
    ...
    return new Promise((fulfill, reject) => {
        ...
        getContents(passon)
            ...
            .then(function (passon) {
                // create direcotry
                fs.mkdir(path.join(passon.localPath, passon.remotePath), function (err) {
                    if (err) {
                        reject(err);
                    }
                    debug('sucessfully created directory');

                    for (i = 0; i < passon.contents.length; i++) {
                        switch (passon.contents[i].type) {
                            case 'file':
                                passon.client
                                    .getFileContents(passon.contents[i].basename)
                                    .then(function (file) {
                                        fs.writeFile(path.join(passon.localPath, passon.remotePath, passon.contents[i].basename), file);
                                    });
                                break;
                            case 'directory':
                                // start syncWebdav for subdirectory
                                passon.remotePath = passon.contents[i].basename;
                                syncWebdav(passon, (err) => {
                                    if (err) {
                                        reject(err);
                                    }
                                });
...
function analyzeContents(passon) {
    return new Promise((fulfill, reject) => {
        ...
        for (i = 0; i < contents.length; i++) {
            if (contents[i].filename === '/bagit.txt') {
                result.bagitCount++;
            }
            if (contents[i].mime === 'application/zip') { //todo: mime does not always exist -> check if it does
                result.zipCount++;
            }
...
function publicShareLoad(passon) {
    return new Promise((fulfill, reject) => {
        ...
        var token = url.parse(passon.share_url).path.split('/')[3];
        ...
        */
        ...
        .then(analyzeContents)
        .then(function(passon){
            //passon.filenames = counter.filenames;
            counter = passon.result;

            // based on file analysis, decide what to do:
            if (counter.length === 0) {
                reject('Public share is empty');
            } else if (counter.bagitCount === 1) {
                copyPublicShare(passon);
            } else if (counter.zipCount === 1) {
                unzipPublicShare(passon);
            } else if (counter.directoryCount === 1) {
                //continue in new directory
                //todo: add path
                passon.path = '';
                publicShareLoad(passon);
            } else {
                //continue as new workspace (not implemented, see content_type)  
            }
            fulfill(passon);
...
function unzipPublicShare(passon) {
    return new Promise((fulfill, reject) => {
                debug('Unzipping %s', passon.id);

        passon.client
            .getFileContents(passon.zipName)
            .then(function(zipData) {
                //change to filename
                fs.writeFileSync('./test.zip', zipData);
            })
            .then(function() {
                //extract zip to dir
                ...
                var cmd = '';
                cmd = 'mv ' + passon.req.file.path + outputPath;
                ...
                exec(cmd, (error, stdout, stderr) => {
                    if (error || stderr) {
                        ...
                        passon.res.status(500).send(JSON.stringify({ error: 'file copy failed: ' + message }));
                        reject(error);
                    } else {
...
function copyPublicShare(passon) {
    return new Promise((fulfill, reject) => {
                debug('Copying %s from public share', passon.id);
        passon.remotePath = '';
        passon.localPath = path.join(config.fs.compendium, passon.id);

        // load all files from public share
        syncWebdav(passon, (err) => {
            if (err) {
                debug('Error syncing webdav directory');
                reject(err);
            } else {
                fulfill(passon);
            }
