...
function getContents(passon) {
    return new Promise((fulfill, reject) => {
        ...
        .catch(function (err) {
            debug(err);
            passon.res.status(404).send(JSON.stringify({ error: 'could not read webdav contents' }));
            reject(err);
        });
    })
}
...
function checkContents(passon) {
    return new Promise((fulfill, reject) => {
        if (passon.result.length === 0) {
            passon.res.status(404).send(JSON.stringify({ error: 'public share is empty' }));
...
function checkAndLoadDirectory(passon) {
    ...
    return new Promise((fulfill, reject) => {
        if (passon.result.directoryCount === 1 && passon.result.bagitCount === 0 && passon.result.zipCount !== 1) {
            passon.res.status(404).send(JSON.stringify({ error: 'single directory found. Use the path parameter to point to the compendium directory' }));
            reject('Aborting: Only a single directory found.');
        } else {
            fulfill(passon);
        }
    });
...
function loadWorkspace(passon) {
    ...
    return new Promise((fulfill, reject) => {
        if (passon.result.directoryCount !== 1 && passon.result.bagitCount === 0 && passon.result.zipCount !== 1) {
            passon.res.status(403).send(JSON.stringify({ error: 'workspace creation not implemented' }));
            reject('Aborting: workspace creation not implemented.');
        } else {
            fulfill(passon);
        }
    });
