...
const debug = require('debug')('loader:slack');
...
joinChannelAndSayHello = function (err, done) {
  ...
  slack.chat.postMessage({
    ...
    text: 'I am now online from *loader* running on host `' + os.hostname() + '`.'
  }, (e, data) => {
    if (e) {
      debug(e);
      err(e);
  ...
  slack.chat.postMessage({
    ...
    if (err) {
      ...
      if (data.ok) {
        ...
        debug('Message send NOT OK. Response for posting new share upload message: %s', JSON.stringify(data));
      }
    }
  });
};
