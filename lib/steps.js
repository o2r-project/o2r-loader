...
function checkAndLoadFiles(passon) {
    return new Promise((fulfill, reject) => {
        if (passon.result.directoryCount === 0 && passon.result.bagitCount === 0 && passon.result.zipCount === 0) {
...
function extractMetadata(passon) {
    return new Promise((fulfill, reject) => {
        ...
        // create stream that logs container output
        let stdStream = Stream.Writable();
        stdStream._write = function (chunk, enc, next) {
            debug('[%s] Extracting metadata     %s', passon.id, Buffer.from(chunk).toString().trim());
            next();
        };
        ...
        let create_options = {
            ...
            //NetworkMode: 'none',
            AutoRemove: true,
            name: 'meta_' + passon.id,
            'Volumes': {
                '/input': {}
              },
              'HostConfig': {
                'Binds': [metaextract_input_dir + ':/data']
            }
        };
        ...
        debug('[%s] Starting Docker container now with options:\n\tcreate_options: %s\n\tstart_options: %s',
            passon.id, JSON.stringify(create_options), JSON.stringify(start_options));

        let cmd = [
            '-debug',
            config.meta.extract.module,
            '--inputdir', '/data',
            '--outputdir', path.join('/data', config.meta.extract.outputDir),
        ...
        debug('[%s] Running metadata extraction with command parameters "%s"', passon.id, cmd.join(' '));
        docker.run(config.meta.toolContainer, cmd, stdStream, create_options, start_options, (err, data, container) => {
            passon.metaContainer = container;
            debug('[%s] container running: %s', passon.id, passon.metaContainer.id);
          if (err) {
            reject(err);
          } else {
            debug('[%s] [run] status code: %s', this.jobId, data.StatusCode);
            if (data.StatusCode === 0) {
                ...
                debug('Error during meta container run: %s', JSON.stringify(data));
        ...
        debug('[%s] Running metadata extraction with command "%s"', passon.id, cmd.join(' '));
        exec(cmd.join(' '), (error, stdout, stderr) => {
            if (error || stderr) {
                debug('[%s] Problem during metadata extraction:\t%s\n\t%s',
                    passon.id, error.message, stderr);
                debug(error, stderr, stdout);
                let errors = error.message.split(':');
                let message = errorMessageHelper(errors[errors.length - 1]);
                error.msg = 'metadata extraction failed: ' + message;
                reject(error);
            } else {
                debug('[%s] Completed metadata extraction:\n%s', passon.id, stdout);
    ...
    return new Promise((fulfill, reject) => {
        ...
        Promise.all([
            new Promise((fulfill, reject) => {
              ...
              debug('[%s] Finished cleanup: %s', passon.id, JSON.stringify(results));
              fulfill(passon);
          });
    });
}
