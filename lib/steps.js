...
function detectCompendium(passon) {
    return new Promise((fulfill, reject) => {
        ...
        if (passon.isBag) {
            passon.detectionFile = path.join(passon.compendium_path, config.bagit.payloadDirectory, config.compendium.detectionFileName);
        } else {
            passon.detectionFile = path.join(passon.compendium_path, config.compendium.detectionFileName);
        }

        try {
            fs.accessSync(passon.detectionFile);

            debug('[%s] Found %s - it\'s a compendium! Content was labeled as "%s"', passon.id, passon.detectionFile, passon.content);
...
function fetchCompendiumID(passon) {
    return new Promise((fulfill, reject) => {
        if (passon.isCompendium) {
            debug('[%s] Reading compendium ID from %s', passon.id, config.compendium.detectionFileName);

            let compendiumID = null;

            // Get compendium yml, or throw exception on error
            try {
                let doc = yaml.safeLoad(fs.readFileSync(passon.detectionFile, 'utf8'));
                compendiumID = doc.id;
                debug('[%s] Successfully read compendium metadata: %O', passon.id, doc);

            } catch (error) {
                debug('[%s] Could not read compendium detection file but content is %s: %s', passon.id, passon.content, error);
                error.msg = 'content_type is ' + passon.content + ' but no compendium provided.';
                error.status = 400;
                reject(error);
            }
            ...
            if (!compendiumID){
                ...
                let error = new Error('No id found in compendium detection file');
                error.status = 400;
                reject(error);
            } else {
                // Validate compendium id
                let idRegex = new RegExp(config.bagtainer.id_regex);
                if (idRegex.test(compendiumID)) {
                    ...
                    Compendium.findOne({ id: compendiumID }).select('id user').exec((err, compendium) => {
                        ...
                        if (compendium === null) {
                            ...
                            let error = new Error('Error fetching ID from compendium, ID already exists');
                            error.status = 400;
                            reject(error);
                        }
                    });
                } else {
                    debug('[%s] Invalid ID %s specified for compendium in %s', passon.id, compendiumID, passon.detectionFile);
                    let error = new Error('Invalid id found in compendium detection file');
                    error.status = 400;
                    reject(error);
                }
            }
        } else {
            debug('[%s] Not an compendium, keeping generated ID', passon.id);
            fulfill(passon);
        }
    });
}
