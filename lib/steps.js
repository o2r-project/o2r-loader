    ...
    return new Promise((fulfill, reject) => {
        ...
        let create_options = Object.assign(
            config.meta.container.default_create_options,
            {
                ...
                HostConfig: {
                    Binds: [metaextract_input_dir + ':' + metaextract_input_dir, metaextract_output_dir + ':' + metaextract_output_dir],
                    AutoRemove: true
                }
            }
        );
        ...
        /*
        docker.run(config.meta.container.image, ['echo $UID'], process.stdout,
            Object.assign(config.meta.container.default_create_options,
                {
                    name: 'meta_testing_' + passon.id,
                    Entrypoint: '/bin/bash'
                }),
        ...
        */
        ...
        docker.run(config.meta.container.image, cmd, containerLogStream, create_options, start_options, (err, data, container) => {
          ...
          if (err) {
            ...
            debug('[%s] extract container status code: %s', this.jobId, data.StatusCode);
