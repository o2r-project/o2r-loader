...
function doBrokering(passon, current_mapping) {
    ...
    return new Promise((fulfill, reject) => {
        exec(cmd, (error, stdout, stderr) => {
            if (error || stderr) {
                ...
                let errors = error.message.split(':');
                let message = errorMessageHelper(errors[errors.length - 1]);
                error.msg = 'metadata brokering failed: ' + message;
                reject(error);
            } else {
    ...
    return new Promise((fulfill, reject) => {
        ...
        if (passon.metadata) {
            if (passon.metadata.raw) {
                // FIXME do actual brokering for o2r metadata
                passon.metadata[config.meta.extract.targetElement] = passon.metadata.raw;

                //doBrokering(passon, 'o2r')
                //    .then(
                doBrokering(passon, 'zenodo')
                    .then(pass => {
                        fulfill(pass);
                    })
                    .catch(err => {
