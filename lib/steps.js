...
function getTextFiles(passon) {
    return new Promise((fulfill, reject) => {
        ...
        fs.readdir(passon.bagpath, (err, files) => {
            if (err) {
                debug('[%s] Error reading compendium data: %s', passon.id, err.message);
                reject(err);
            }

            files.map(function (file) {
                return path.join(passon.bagpath, file);
            }).filter(function (file) {
                return fs.statSync(file).isFile();
            }).forEach(function (file) {
                if (textFileRegex.test(path.extname(file))) {
                    passon.textFiles.push(path.extname(file));
                }
            });
        });

        if (passon.textFiles.length === 0) {
            debug('[%s] No text files found ', passon.id);
            fulfill(passon);
        } else {
            debug('[%s] Found text files: %s ', passon.id, passon.textFiles.toString());
            fulfill(passon);
        }
    });
}
...
function checkEncoding(passon) {
    return new Promise((fulfill, reject) => {
        ...
        passon.textFiles.forEach(function(element) {
            const fileBuffer = fs.readFileSync(element);
            const charsetMatch = detectCharacterEncoding(fileBuffer);

            // If encoding type is not supported (e.g. ISO)
            if (config.encoding.supportedEncodings.indexOf(charsetMatch.encoding) === -1) {
                debug('Unsupported encoding %s found in file %s!', charsetMatch.encoding, element);
                let err = new Error('Encoding not supported');
                err.status = 404;
                reject(err);
                return;
            }
