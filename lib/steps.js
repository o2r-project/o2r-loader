    ...
    return new Promise((fulfill, reject) => {
        ...
        if (passon.metadata) {
            if (passon.metadata.raw) {
                ...
                let metabroker_input_dir = passon.metadata_dir;
                let metabroker_output_dir = passon.metadata_dir;
                let cmd = [
                    config.meta.cliPath,
                    '-debug',
                    config.meta.broker.module,
                    '--inputdir', metabroker_input_dir,
                    '--map', config.meta.broker.mappings[current_mapping].mappingFile,
                    '--outputdir', metabroker_output_dir
                ].join(' ');
                ...
                exec(cmd, (error, stdout, stderr) => {
                    if (error || stderr) {
                        ...
                        fs.readdir(metabroker_output_dir, (err, files) => {
                            if (err) {
                                debug('[%s] Error reading brokered metadata directory %s:\n\t%s', passon.id,
                                    metabroker_output_dir, err);
                                reject(err);
                            } else {
                                ...
                                fs.readFile(config.meta.broker.mappings['zenodo'].mappingFile, (err, data) => {
                                    if (err) {
                                        debug('[%s] Error reading mapping file: %s', passon.id, err.message);
                                        reject(err);
                                    } else {
                                        let mapping = JSON.parse(data);
                                        let mapping_output_file = path.join(metabroker_output_dir, mapping.Settings.outputfile);
                                        ...
                                        fs.readFile(mapping_output_file, (err, data) => {
                                            if (err) {
                                                ...
                                                // read mapped metadata to passon for saving to DB
                                                passon.metadata[config.meta.broker.mappings[current_mapping].targetElement] = mapping_output;
