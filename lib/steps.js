...
function getTextFiles(passon) {
    return new Promise((fulfill, reject) => {
        ...
        //recursively iterate through bagpath and collect file names
        recursiveReaddir(passon.bagpath, [ignoreFunc], function (err, files) {
            if (err) {
    ...
    return new Promise((fulfill, reject) => {
        ...
        for (let i = 0; i < passon.textFiles.length; i++) {
            ...
            if (config.encoding.supportedEncodings.indexOf(charsetMatch.encoding) === -1) {
                invalidFiles.push({file: element, encoding: charsetMatch.encoding});
            }
        }

        if (invalidFiles.length !== 0) {
            debug('Unsupported encoding found in file(s) %s!', invalidFiles.toString());
            let err = new Error('unsupported encoding');
            err.status = 422;
            err.msg = 'files with unsupported encoding detected: ' + JSON.stringify(invalidFiles);
            reject(err);
        } else {
