    ...
    return new Promise((fulfill, reject) => {
        ...
        for (let i = 0; i < contents.length; i++) {
            if (contents[i].basename === 'bagit.txt') {
                ...
                result.zipCount++;
                if (passon.zipFile) {
                    //use the file provided in the path parameter
    ...
    return new Promise((fulfill, reject) => {
        if (passon.result.directoryCount === 0 && passon.result.bagitCount === 0 && passon.result.zipCount === 0) {
            ...
            if (passon.isZipFile) {
                ...
                downloadURL = passon.shareURL + '/download?path=/&files=' + encodeURIComponent(path.basename(passon.webdav_path));
            } else {
                //wget zip and continue with unzip
                downloadURL = passon.shareURL + '/download?path=' + encodeURIComponent(passon.webdav_path);
            }
...
function checkEncoding(passon) {
    return new Promise((fulfill, reject) => {
        ...
        if (invalidFiles.length !== 0) {
            debug('[%s] Unsupported encoding found in file(s) %s!', passon.id, JSON.stringify(invalidFiles));
...
function extractMetadata(passon) {
    return new Promise((fulfill, reject) => {
        ...
        if(config.fs.volume) {
            debug('[%s] volume is configured, overwriting binds configuration (was %s)', passon.id, JSON.stringify(binds));
            // passon.compendium_path always starts with config.fs.base
            binds = [
                config.fs.volume + ':' + config.fs.base
            ];
        }
        debug('[%s] Binds: %s', passon.id, JSON.stringify(binds));
        ...
        docker.run(config.meta.container.image, cmd, containerLogStream, create_options, start_options, (err, data, container) => {
            ...
            if (err) {
                ...
                if (data.StatusCode === 0) {
                    debug('[%s] Completed metadata extraction: %s', passon.id, JSON.stringify(data));
                    ...
                    debug('[%s] Error during meta container run: %s', passon.id, JSON.stringify(data));
                    container.logs({
                        follow: true,
                        stdout: true,
                        stderr: true,
